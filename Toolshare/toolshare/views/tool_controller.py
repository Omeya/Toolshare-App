...
class ToolController(BaseController):
    ...
    def tool_description(request, tool_id):
        if request.method == 'GET':
            ...
            return render(request, 'toolshare/tool-description.html', {
                'tool':tool
            })
    ...
    def change_tool_info(request, tool_id):
        ...
        if request.method == 'POST':
            ...
            if changeToolInfo.is_valid():
                ...
                if newtool.shed is not None and newtool.pickup_location == 'At Shed':
                    tool.pickup_location = 'At Shed'
                    tool.shed = newtool.shed
                else:
                    tool.pickup_location = 'My home'
                    tool.shed = None
                tool.save()
    ...
    def change_availability_tool(request, tool_id):
        ...
        if request.method == 'POST':
            ...
            if change_avail_form.is_valid():
                ...
                now = timezone.now()
                #current_approved_reservations = Reservation.objects.filter(tool_id = tool.id, status ="A", start_date__lt = now, end_date__gt = now )
        ...
        # Get all the current-approved reservations
